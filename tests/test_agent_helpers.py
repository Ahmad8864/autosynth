...
def candidate():
    return Candidate(
        candidate_id="c1", domain="qa", source_id="s1",
...
def test_challenger_build_request_is_deterministic(domain, grounding):
    req1 = challenger.build_request(
        item_id="i1", round_n=1, model_key="mock/foo",
        grounding=grounding, feedback=[], prior_payloads=[], domain=domain,
    )
...
def test_challenger_build_request_injects_harness_rules(domain, grounding):
    ...
    req = challenger.build_request(
        item_id="i1", round_n=1, model_key="mock/foo",
        grounding=grounding, feedback=[], prior_payloads=[], domain=domain, harness=h,
    )
...
def test_challenger_parse_response_caps_rubric_weight():
    ...
    cand = challenger.parse_response(text, source_id="s1", round_n=1,
                                     domain_name="qa", rubric_max_weight=7)
    assert cand.rubric[0].weight == 7


def test_challenger_parse_response_handles_garbage_rubric_weights():
    text = json.dumps({
        "payload": {"question": "Q?"}, "reference_output": "r",
...
def test_solver_build_request_role_validation(domain, candidate):
    with pytest.raises(ValueError):
        solver.build_request(
            item_id="i1", round_n=1, attempt=0, model_key="mock/x",
            candidate=candidate, role="judge", domain=domain,
        )


def test_solver_build_request_distinct_ids_per_attempt(domain, candidate):
    a0 = solver.build_request(item_id="i1", round_n=1, attempt=0, model_key="m",
                              candidate=candidate, role="weak", domain=domain)
    a1 = solver.build_request(item_id="i1", round_n=1, attempt=1, model_key="m",
                              candidate=candidate, role="weak", domain=domain)
    sweak = solver.build_request(item_id="i1", round_n=1, attempt=0, model_key="m",
                                 candidate=candidate, role="strong", domain=domain)
...
def test_quality_build_request_is_json_mode(domain, candidate):
    req = verifier.build_quality_request(item_id="i1", round_n=1, model_key="m",
...
def test_judge_build_request_keyed_on_parent_id(domain, candidate):
    j1 = verifier.build_judge_request(
        item_id="i1", round_n=1, attempt=0, model_key="m", candidate=candidate,
        solver_response="resp-a", solver_role="weak", domain=domain,
        parent_response_id="parent-A",
    )
    j2 = verifier.build_judge_request(
        item_id="i1", round_n=1, attempt=0, model_key="m", candidate=candidate,
        solver_response="resp-b", solver_role="weak", domain=domain,
...
def test_judge_parse_uses_weighted_average_when_total_missing(candidate):
    ...
    s = verifier.parse_judge(text, candidate=candidate, solver_role="strong",
                             attempt=0, solver_response_text="x")
...
def test_judge_parse_clamps_and_drops_bad_per_criterion(candidate):
    ...
    s = verifier.parse_judge(text, candidate=candidate, solver_role="weak",
                             attempt=0, solver_response_text="x")
...
def test_reflector_build_request_summarizes_priors(candidate):
    rounds = [
        Round(
            refinement_round=1, candidate=candidate,
            quality=QualityCheck(passed=True),
            evaluation=EvalReport(weak_avg=0.8, strong_avg=0.9, gap=0.1, accepted=False,
                                  rejection_reasons=["weak_avg too high"]),
        ),
    ]
    req = reflector.build_request(
        item_id="i1", round_n=2, model_key="m",
        prior_rounds=rounds, domain_name="qa", leakage_rules=["x"],
