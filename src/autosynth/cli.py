...
app = typer.Typer(
    ...
    help="autosynth: agentic synthetic data generation",
)
console = Console()
...
def run_cmd(
    ...
    console.print(f"[dim]export with: autosynth export --run {runner.run_dir} --format jsonl[/dim]")
...
def _render_domain_template(*, name: str, cls: str, path: Path) -> str:
    ...
    src = files("autosynth.templates").joinpath("new_domain.py.tmpl").read_text(encoding="utf-8")
